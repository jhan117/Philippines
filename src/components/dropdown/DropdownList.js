...
const Dropdown = ({ className, text, item, href, isOpen, onToggle, onClose }) => {
  ...
  return (
    <Fragment>
      ...
      <li className={`${className} relative`}>
        <button className="flex items-center gap-1 hover:text-primary-400 transition-colors" onClick={onToggle}>
          {text}
          <Image src={DROP_DOWN} width={20} height={20} alt="dropdown icon" className={`transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
        </button>
        <ul
